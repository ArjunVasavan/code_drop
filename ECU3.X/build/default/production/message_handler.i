...
# 3 "message_handler.c" 2
# 1 "./message_handler.h" 1
# 10 "./message_handler.h"
typedef enum {
    e_ind_off,
    e_ind_left,
    e_ind_right,
...
# 4 "message_handler.c" 2
# 1 "./msg_id.h" 1
# 5 "message_handler.c" 2
# 1 "./can.h" 1
# 31 "./can.h"
void init_can(void);
...
# 7 "message_handler.c" 2
unsigned long int timer_count;
unsigned char event[9][3] = {"ON","GN","G1","G2","G3","G4","G5","Gr","C_"};
void display_labels(void) {
    clcd_print("SP", (0x80 + (0)));
    clcd_print("GR", (0x80 + (4)));
    clcd_print("RPM", (0x80 + (8)));
    clcd_print("IND", (0x80 + (13)));
}
void handle_speed_data(uint8_t *data, uint8_t len) {
...
void handle_gear_data(uint8_t *data, uint8_t len) {
    if(*data < 9)
        clcd_print(event[*data], (0xC0 + (4)));
}
void handle_rpm_data(uint8_t *data, uint8_t len) {
    data[len] = '\0';
    clcd_print(data, (0xC0 + (8)));
}
void handle_engine_temp_data(uint8_t *data, uint8_t len) {
}
void handle_indicator_data(uint8_t *data, uint8_t len) {
    ...
    if(c_flag == 0)
    {
        if(msg_id == 0x10) {
            ...
            if(*data == 8)
            {
                c_flag = 1;
                clcd_clear();
                clcd_print("Collision !", (0x80 + (0)));
                clcd_print("Vehicle Damaged", (0xC0 + (0)));
            }
        } else if(msg_id == 0x30) {
