...
int main(int argc, char **argv)
{
    ...
    db_state_t state = DB_EMPTY;  // Track database state
    ...
    create_hash_table(hash_table_array);  // Initialize all hash table entries
    ...
    while (1) {
        ...
        switch (choice) {
            case 1:
                // Allow creation only if DB is empty or recently updated
            ...
            case 5:
                // Update allowed only when DB is empty (load from backup)
                if (state == DB_EMPTY) {
                ...
                break;
            case 6:
                free_database(hash_table_array);  // Cleanup before exit
                return SUCCESS;
            default:
