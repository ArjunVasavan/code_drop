...
uint16_t get_rpm()
{
    ...
    unsigned int rpm = ((unsigned long)read_adc(0x04) * 6000) / 1023;
    int i = 0, num = rpm;
    if(num == 0) { data[i++] = '0'; }
    else {
...
IndicatorStatus process_indicator()
{
    ...
    if(key != 0x0F) {
        if(key == 0x0E) indicator = e_ind_left;
        else if(key == 0x0D) indicator = e_ind_right;
        else if(key == 0x0B) indicator = e_ind_hazard;
        else if(key == 0x07) indicator = e_ind_off;
    }
    can_transmit(0x50, (uint8_t*)&indicator, 1);
    return indicator;
}
